...
- **電子報訂閱**：你訂閱時留下的 Email，用於寄送電子報內容給你。你可以隨時透過每封電子報信末的取消訂閱連結退出，退出後我們會保留該筆 Email 記錄但標記為已取消訂閱、不會再寄信給你；如需完全刪除這筆記錄，歡迎透過[聯絡頁面](/contact)告知我們。 每封電子報信件都是系統自動寄送，不會有人監看回信信箱，請勿直接回信；信件最下方會附上隱私權政策與服務條款的連結，方便你隨時查閱。
...
本網站提供以下功能：部落格文章閱讀、兒少委員專區（含時間軸與相關文件）、留言板、聯絡表單、學生意見箱、以及線上問卷（填寫、瀏覽統計結果）。站長保留隨時新增、修改或終止任何功能的權利，不另行個別通知。
...
create table if not exists newsletters (
  ...
  body text not null,
  content text not null,
  type text not null default 'news' check (type in ('news', 'issue', 'survey')),
